...
class SwiftLoadingTool: UIView {
    ...
    /// 最小文字宽度
    static let minTextWidth: CGFloat = 200.0
    /// 最大文字宽度
    static let maxTextWidth: CGFloat = kScreenWidth
    /// 是否自动隐藏 对文字框有效 默认自动隐藏
    static var isAutoHidden: Bool = true
    /// 自动隐藏时间 对文字框有效 默认1秒隐藏 需要先设置isAutoHidden属性
    static var autoHiddenTime: Double = 2.0
    /// 展示位置 对文字框有效 默认在中间
    static var positionType: LoadingPositionType = LoadingPositionType.centerType
    /// 文字框圆角大小 默认5
    static var textCornerRadius: CGFloat = 6.0
    /// 文字框背景颜色 黑色
    static var textViewBgColor: UIColor = UIColor.lightGray
    /// 文字大小
    ...
    func updateLabelFrame(text: String?, maxSize: CGSize) {
        ...
        let tempWidth = text!.DS_widthWithString(font: UIFont.systemFont(ofSize: 14), size: CGSize(width: 0, height: 20))

        let size = text?.getSize(maxSize: maxSize)
        var newTextFrame = tool.alertLabel.frame
        newTextFrame.size.width = tempWidth+20
