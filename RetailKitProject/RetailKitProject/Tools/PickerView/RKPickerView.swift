...
//  RetailKitProject
//
//  Created by Sammy Chen on 4/12/2019.
...
/// - date//时间:
/// - gender//性别:
public enum RKJPickerViewStyle {
     case address  // 地址
     case date     // 时间
     case gender   // 性别
     case position // 位置
...
class RKPickerView: UIView , UIPickerViewDelegate, UIPickerViewDataSource{
     
     
     var pickerDelegate : PickerDelegate?
     private var pickerStyle: RKJPickerViewStyle?
     private let pickerH : CGFloat! = 260 * ScreenScaleY
     private var addressPicker : UIPickerView = UIPickerView()
     private var datePicker : UIDatePicker = UIDatePicker()
     private var genderPicker : UIPickerView = UIPickerView()
     private var positionrPicker : UIPickerView = UIPickerView()
     ...
     private var selectedPosition : String = String()

     var isAddress : Bool?
     
     // MARK: - 初始化UI
     init(_ delegate : PickerDelegate,_ arr : NSMutableArray,_ style : RKJPickerViewStyle) {
          ...
          view.backgroundColor = RGBColor_A(230, 230, 230, 1)
          ...
          titleLabel.text = "选择门店"
          titleLabel.textColor = MainColor
          titleLabel.textAlignment = .center
          titleLabel.frame = CGRect.init(x: cancelButton.frame.maxX, y: 0, width: kScreenWidth-120, height: 44)
          ...
          backgroundButton.backgroundColor = RGBColor_A(0, 0, 0, 0)
          ...
          case .date:
               ...
               datePicker.addTarget(self, action: #selector(RKPickerView.dateSelected(_:)), for: .valueChanged)
          ...
          case .position:
                positionrPicker = UIPickerView.init(frame: CGRect.init(x: 0, y: 44, width: kScreenWidth, height: pickerH - 44))
                positionrPicker.delegate = self
                positionrPicker.dataSource = self
                positionrPicker.backgroundColor = UIColor.white
                isAddress = false
                self.addSubview(positionrPicker)
            }
          if pickerStyle != RKJPickerViewStyle.date {
               switch isAddress {
               case true:
                    self.getAddressData()
               default:
//                    dataArray = NSMutableArray.init(array: ["男","女"])
     ...
     @objc func doneButtonClick() {
          ...
          }else if pickerStyle == .position {
            if selectedPosition == "请选择门店" {
                pickerDelegate?.selectedNonPosition(self, selectedPosition)
                return
            }
               pickerDelegate?.selectedPosition(self, selectedPosition)
          }
          self.pickerViewHidden()
     }
     /// 时间选择
     ...
     public func pickerViewShow() {
          ...
          UIView.animate(withDuration: 0.4, animations: {
            self.backgroundButton.backgroundColor = RGBColor_A(0, 0, 0, 0.4)
     ...
     func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
          ...
          default:
               ...
               selectedPosition = dataArray[row] as! String
          }
     }
}
