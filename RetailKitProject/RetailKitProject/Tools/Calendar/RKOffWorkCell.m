...
- (void)setModelDict:(NSDictionary *)modelDict {
    ...
    NSString *time = [NSString stringWithFormat:@"%@",_modelDict[@"off_time"]];
    _checkLabel.text = [NSString stringWithFormat:@"打卡时间：%@",[NSDate getTimeFromTimestamp:time.integerValue]];
    
    _locationLabel.text = [NSString stringWithFormat:@"%@",_modelDict[@"off_address"]];

    _photo = [NSString stringWithFormat:@"%@",_modelDict[@"off_photo"]];
...
if (self.offCellPhotoBlock) {
    self.offCellPhotoBlock(_photo);
}
...
//    }];
    ...
    [_checkLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@(self.checkLabel.frame.size.height));
        make.left.equalTo(self.contentView).offset(36+20*ScreenScale+30);
    ...
    [locaImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.checkLabel);
